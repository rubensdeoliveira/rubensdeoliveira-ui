...
export function Button({
  ...
  const sharedClassName = `flex items-center gap-2 ${className || ''}`

  switch (variant) {
    case 'secondary':
      return (
        <button {...rest} className={`${sharedClassName} border focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2`}>
          {renderButtonContent}
        </button>
      )
    case 'link':
      return (
        <button
          {...rest}
          className={`${sharedClassName} font-medium`}
        >
          {renderButtonContent}
        </button>
      )
    case 'ghost':
      return (
        <button {...rest} className={`${sharedClassName} bg-transparent border-0 p-2`}>
          {renderButtonContent}
        </button>
      )
    case 'google':
    ...
    default:
      return (
        <button {...rest} className={`${sharedClassName} focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none`}>
          {renderButtonContent}
        </button>
      )
  }
